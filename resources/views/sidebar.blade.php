<aside class="bg-grey-lightest p-6 pr-10 border-l border-r w-64">
    ...
    <div class="widget border-b-0">
        @if (auth()->check())
            @if(auth()->user()->confirmed)
                <button class="btn is-green w-full" @click="$modal.show('new-thread')">Add New Issue</button>
            @else
    ...
    <div class="widget">
        ...
        <ul class="list-reset text-sm">
            <li class="pb-3">
                <a href="/issues" class="flex items-center text-grey-darkest hover:text-blue hover:font-bold {{ Request::is('threads') && ! Request::query() ? 'text-blue font-bold' : '' }}">
            ...
            @if (auth()->check())
                <li class="pb-3">
                    <a href="/issues?by={{ auth()->user()->username }}"
            ...
            <li class="pb-3">
                <a href="/issues?popular=1" class="flex items-center text-grey-darkest hover:text-blue hover:font-bold {{ Request::query('popular') ? 'text-blue font-bold' : '' }}">
            ...
            <li>
                <a href="/issues?unanswered=1" class="flex items-center text-grey-darkest hover:text-blue hover:font-bold {{ Request::query('unanswered') ? 'text-blue font-bold' : '' }}">
